 ...
 * @author Tjut team
 */
public abstract class BaseServiceImpl2<M extends BaseMapper<T>, T>  implements BaseService<T> {
    @Autowired
    protected M baseDao;
    ...
    protected <T> PageData2<T> getPageData(List<?> list, long total, Class<T> target){
        List<T> targetList = ConvertUtils.sourceToTarget(list, target);

        return new PageData2<>(targetList, total);
    }

    protected <T> PageData2<T> getPageData(IPage page, Class<T> target){
